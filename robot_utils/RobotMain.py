...
from tools import ClassifierSVMLoader
...
class RobotMain:
    def __init__(self, robot, wall_avoider, room_controller, room_detector):
        self._robot = robot
        self._wall_avoider = wall_avoider
        self._room_controller = room_controller
        self._room_detector = room_detector
    ...
    def notify(self, value):
        ...
        if now - self._last_notified > TIME_BETWEEN_NOTIFICATIONS:
            ...
            if self._status == RobotStatus.PASSAGE:
                ...
                cv = self._robot.ultrasound()
                room = self._room_detector.predict((lv, rv, cv))
                print("I'm leaving room: {}".format(room+1))
...
def main():
    ...
    room_detector = ClassifierSVMLoader('../results/svm-params-1462823669_34')
